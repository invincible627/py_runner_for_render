...
async def handle_user_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "input" in user_message_content_text.replace(" ", "").lower():
        ...
        await log_event(user_id_message, f"user tried to use input() in: {user_message_content_text}", user_metadata_message)
        return
...
async def handle_document_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await check_banned_users(user_id_doc):
        user_metadata_doc = "you are banned from bot by admin"
        await update.message.reply_text(user_metadata_doc)
        await log_event(user_id_doc, f"bot sent: {ban_reply_run_file}", user_metadata_doc)
        return
    ...
    try:
        ...
        async with aiofiles.open(save_path, "r", encoding='utf-8') as f:
            ...
            if "input(" in content:
                ...
                await log_event(user_id_doc, "user uploaded a file with input", user_metadata_doc)
